...
export default function SeatsForm({
  ...
  //State for error messages, errorMsg can contain multiple error codes as strings and be used to show different error messages
      ...
      <div className="bg-[#252525dc] border-2 border-blue-500 w-[80vw] lg:w-[35vw] h-[50vh] rounded-2xl flex flex-col items-center">
        ...
        <div className="flex lg:w-[30vw] w-[60vw] h-[35vh] justify-around m-[1vw]">
          <form className="flex flex-col align-middle gap-1 w-[80%] lg:w-[17vw] self-center items-center">
            <label htmlFor="userName" className="text-[1rem] text-center text-white">
              Ingrese su nombre
            </label>
            <input
              ref={userName}
              ...
              placeholder="Ej: Juan"
              id="userName"
              className="border-[#2659e4] border-2 rounded-[12px] text-center w-[85%] text-white"
            />
            <p className="form-errors">
              {errorMsg.includes('errorName') ? 'El nombre es incorrecto' : ' '}
            </p>

            <label htmlFor="lastName" className="text-[1rem] text-center text-white">
            ...
            <label htmlFor="mail" className="text-[1rem] text-center text-white">
              Ingrese su mail
            </label>
            <input
              ref={mail}
          ...
          <section className="hidden lg:w-[10vw] h-[25vh] overflow-y-auto lg:flex flex-col gap-4 self-center">
            <h2 className="text-xl text-white text-center">Tus tickets</h2>
            {
              ...
              selectedSeats.map((showSeat, i) => (
                <Ticket
                  ticketNum={i + 1}
                  seatNum={showSeat.seatNum}
                  lineNum={showSeat.lineNum}
                  movieDate="16/10/25"
                  movieImage="exampleFilmImg.png"
                  movieTitle="Titulo de la pelicula"
                  style="w-[100%] min-h-[6vh] h-[6vh]"
                  key={showSeat._id}
                />
              ))
            }
        ...
        <section className="h-[10vh] lg:h-[5vh] flex-col-reverse sm:flex-row items-center flex justify-center gap-5 w-[80%]">
          <button
            className=" max-h-[6vh] w-[40%] md:w-[10vw] h-[4vh] bg-gray-600 border-2 border-blue-400 text-white rounded-xl"
            onClick={close}
          >
            Cancelar
          </button>
          <button
            ...
            className="bg-blue-400 text-white border-2 border-white max-h-[6vh] h-[4vh] w-[40%] md:w-[10vw] m-[0%] rounded-xl"
          >
            Siguiente
          </button>
        </section>
        {errorMsg.includes('errorEmpty') ? (
          <p className="text-red-500 text-sm self-center">Debes completar todos los campos</p>
        ) : null}
      </div>
    </main>
  );
